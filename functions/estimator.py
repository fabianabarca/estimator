...
def calcular_delay(group):
    ...
    group['arrival_time'] = pd.to_datetime(
        group['arrival_time'], format='%H:%M:%S')
...
def calcular_polinomios(stops_measurement):
    ...
    stops_measurement = stops_measurement.groupby(
        ['trip_id', 'date']).apply(calcular_delay)
    ...
    stops_measurement['trip_departure_time'] = stops_measurement.groupby(
        'trip_id')['arrival_time'].transform('first')
    
    combinations = stops_measurement[[
        'route_id', 'service_id', 'shape_id', 'stop_id']].drop_duplicates()
    ...
    for i, combination in combinations.iterrows():
        ...
        if not subset.empty:
            x_values = pd.to_datetime(subset['trip_departure_time'], format='%H:%M:%S').dt.hour * 3600 + \
                ...
                pd.to_datetime(subset['trip_departure_time'],
                               format='%H:%M:%S').dt.second
...
def estimator(route_id, service_id, shape_id, start_time, polynomials, route_stops):
    ...
    sequence_of_stops = get_sequence_of_stops(
        route_id, shape_id, route_stops)
    ...
    for stop_id in sequence_of_stops:
        if (route_id, service_id, shape_id, stop_id) in polynomials:
            ...
            polynomial_function = polynomials[(
            ...
            estimated_arrival = departure_time + \
                pd.Timedelta(seconds=estimated_delay)

            # Guardar el tiempo estimado de llegada para esta parada en el diccionario
            estimated_arrival_times[stop_id] = estimated_arrival.strftime(
                '%H:%M:%S')
        else:
...
def generate_stop_times_df(stops_measurement, route_stops, trip_times, trips):
    ...
    for index, row in trip_times.iterrows():
        ...
        service_id = trips.loc[trips["trip_id"]
                               == trip_id, "service_id"].values[0]
        ...
        timepoint_values = [
            1 if i == 0 else 0 for i in range(len(sequence_of_stops))]
